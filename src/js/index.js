...
function buildHomepageHTML(releasesJson, jckJSON, OS) {
  ...
  if (OS) {
    releasesJson.forEach((eachAsset) => {
        ...
        if (matchingFile == null) {
          ...
          if (Object.keys(jckJSON).length !== 0) {
            if (
              ...
              document
                .getElementById("jck-approved-tick")
  ...
  if (matchingFile) {
    if (matchingFile.binary.installer) {
      ...
      dlVersionText.innerHTML += ` - ${Math.floor(
        matchingFile.binary.package.size / 1000 / 1000
      )} MB`;
    }
    ...
    dlLatest.href = `./releases.html?${makeQueryString({
      variant,
      jvmVariant,
  ...
  dlLatest.className = dlLatest.className.replace(
    /(?:^|\s)invisible(?!\S)/g,
    " animated "
  );
  dlOther.className = dlOther.className.replace(
    /(?:^|\s)invisible(?!\S)/g,
    " animated "
  );
  dlArchive.className = dlArchive.className.replace(
    /(?:^|\s)invisible(?!\S)/g,
    " animated "
  );

  dlLatest.onclick = () => {
    document.getElementById("installation-link").className +=
      " animated pulse infinite transition-bright";
  };
  ...
  const template = Handlebars.compile(
    document.getElementById("template-video").innerHTML
  );
  document.getElementById("video").innerHTML = template({
    sampleYoutube,
  });
}
